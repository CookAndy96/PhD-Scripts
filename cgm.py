...
def weighted_percentile(data, weights, perc, key="Unspecified Error key..."):
	...
	sorted_data, sorted_weights	= np.array(data)[ind_sorted], np.array(weights)[ind_sorted]

	# Remove nan entries
	whereDataIsNotNAN, whereWeightsIsNotNAN	= np.where(np.isnan(sorted_data) == False), np.where(np.isnan(sorted_weights) == False)
...
def avgWeighted(x, y, min, max, s, inc, weight):       #x and y are datasets, min and max is the minimum and maxixum x value to measure from and to, s is the starting position of the data, inc is the total
	...
	med, upper, lower, radii, nbins = np.zeros(inc-1), np.zeros(inc-1), np.zeros(inc-1), np.zeros(inc-1), np.divide(1, inc)
...
def colDensDist(ion, load=load):
	...
	for n in range(len(ion['x'])):
		for m in range(len(ion['y'])):
			load.data[str(ion) + 'rad'][n, m] = np.sqrt(np.add(ion['x'][n]**2,ion['y'][m]**2))
...
def galRad(galradfac, virialradius):
	return np.maximum(galradfac * virialradius, 0.01)
...
def saveData(filename, header, *x):
	f = open(f'./highResSims/{filename}.csv', 'w')
	writer = csv.writer(f)
...
def coolingTime(load, normRadius, min, max):
	...
	coolinggas, = np.where((load.gcol[:len(load.ne)]<0) & (load.sfr[:len(load.ne)] == 0) & (normRadius[:len(load.ne)] >= min) & (normRadius[:len(load.ne)] <= max) & (load.halo[:len(load.ne)] == 0) & (load.subhalo[:len(load.ne)] <= 0))
	meanweightion = 1.2
	...
	lcool = tcool * csound/kpcincm*1e3
...
def starBirth(load):
	...
	birthRedshift = np.divide(1,stellarAge) - 1
	birthMass = np.multiply(load.data['gima'][whereStars], 1e10)
	return birthRedshift, birthMass
...
def data_acq(data_path, pos):
	ld = open(str(data_path), 'r')
	lines=ld.readlines()

	ion = np.zeros(len(lines))
	observed_stel_mass = []
	observed_impact_param = []

	for x,y in zip(lines,range(len(lines))):
		ion[y] = float(x.split(',')[pos])
	ld.close()

	return np.array(ion)

def avg(x, y, min, max, inc):
	nbins = np.divide(np.subtract(max, min), inc)
	pres, = np.where((x >= min) & (x < np.add(min, abs(nbins))))
	med, upper, lower, radii = [median(y[pres])], [np.nanpercentile(y[pres], 84)], [np.nanpercentile(y[pres], 16)], [0]

	for i in np.linspace(min, max, inc):
		...
		if len(pres) > 0:
			...
			radii.append(np.add(i,abs(nbins)))

	return med, upper, lower, radii

def histasline(y, weight, numbins, mu, sigma):
	prob = mlab.normpdf(bins, mu, sigma)
	return bins, prob
